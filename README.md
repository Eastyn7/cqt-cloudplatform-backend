...
存储服务队的部门组织结构，如宣传部、组织部、外联部等，用于成员分组、活动组织与前端展示排序。
...
| 字段名          | 数据类型     | 是否必填           | 说明                                        |
| --------------- | ------------ | ------------------ | ------------------------------------------- |
| `dept_id`       | int          | 是（PK，自增）     | 部门唯一标识                                |
| `dept_name`     | varchar(50)  | 是                 | 部门名称                                    |
| `description`   | varchar(255) | 否                 | 部门简介                                    |
| `leader_id`     | varchar(20)  | 否                 | 部门负责人学号，对应 `auth_info.student_id` |
| `display_order` | int          | 否（默认：0）      | 展示排序值（值越大越靠前）                  |
| `created_at`    | datetime     | 是（默认当前时间） | 创建时间                                    |
| `updated_at`    | datetime     | 是（自动更新）     | 更新时间                                    |
...
5.9.5 `autoUploadService.ts`（自动分类上传逻辑）
...
5.9.7 `signedUrlService.ts`（临时访问链接）【前端直传替换了】
          ...
          "members": [
            {
              ...
              "position": "部长",
              "photo_url": "https://example.com/photos/yyy.jpg",
